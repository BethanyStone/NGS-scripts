...
# reference genome needs to be indexed using bismark_genome_preparation command, specifying bowtie2 option (see top of script)
...
# to run bismark using bowtie1 (not used, but option there if needed - just hash out bowtie2 command):
...
# methylation extraction with full cytosine report (not used - unhash to use if needed) 
...
# SINGLE END Epignome
######################################
All this does differently is trip the first 6bp from the fastq reads per epicentre protocol to eliminate issues of random hexamer binding
...
cd 4_bismark_alignment
#bismark --bowtie1 --sam -n 2 -l 20 ../../$genome_path ../2_trimgalore/${fq_file%%.fastq*}_trimmed.fq* 2>&1 | tee -a ../${fileID}_logs_${dow}.log
bismark --bowtie2 --sam -N 0 -L 20 ../../$genome_path ../2_trimgalore/${fq_file%%.fastq*}_trimmed.fq* 2>&1 | tee -a ../${fileID}_logs_${dow}.log
